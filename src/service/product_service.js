...
export const getAllProducts = async (data) => {
  try {
    const res = await apiClient.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/products/all-products`,
      data
    );
...
export const getProductBySlug = async (slug) => {
  try {
    const res = await apiClient.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/products/product/by-slug/${slug}`
    );
...
export const getAllCategories = async () => {
  try {
    const res = await apiClient.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/categories/get-all-categories`
    );
...
export const getProductsByCategory = async (categoryId) => {
  try {
    const res = await apiClient.get(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/categories/get-category/${categoryId}`
    );
    return res.data;
  } catch (error) {
